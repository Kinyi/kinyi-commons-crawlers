...
public class NumberRecognizer {
    ...
    public static char recognizeNumber(boolean[] verticalLineArray) throws Exception {
        ...
        if (verticalLineArray[6 - 1]) {// 2 , 3,  7
            if (verticalLineArray[8 - 1]) {// 2, 3
                if (verticalLineArray[15 - 1]) {//3
                    return '3';
                } else {
                    return '2';
                }
            ...
            if (verticalLineArray[7 - 1]) {//8
                return '8';
            } else {
                if (verticalLineArray[8 - 1]) {//1 , 9 , 0
                    if (verticalLineArray[10 - 1]) {//9 , 0
                    ...
                    if (verticalLineArray[9 - 1]) {// 6
                        return '6';
                    } else {// 4 ,5 , '.' , ','
                        if (verticalLineArray[12 - 1]) {// 4
                            return '4';
                        } else {
    ...
    public static void initialize(Map<Integer, String> imageNumerMap) throws Exception {
        ...
        BufferedImage img = tbimg.getImg("/Users/Kinyi_Chan/num.png");
        ...
        for (int i = 0; i < width; i++) {
            ...
            if (!isBlankLine(tempVL)) {//不是空白行
                if (isFirstUnblank) {//是数字的第一竖列
